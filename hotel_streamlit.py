...
dataset1 = pd.read_csv('C:\File\Kuliah\SEMESTER6\VisDaat\TubesFinal\Hotel Reservations.csv')
...
target_option = ['room_type_reserved', 'market_segment_type', 'booking_status', 'type_of_meal_plan']
target_select = st.selectbox("Tentukan target yang  diinginkan", options=target_option)
...
def select_columns(dataset: pd.DataFrame, threshold: int = 10) -> list[str]:
    ...
    return discrete_columns, continuous_columns, target_select

discrete_columns, continuous_columns, target_select = select_columns(dataset1)
...
def visualizations_discrete(dataset: pd.DataFrame):
    ...
    for index, column in enumerate(discrete_columns):
        ...
        # Plot countplot
        sns.countplot(data=dataset, x=column, hue=target_select, ax=axes[index * 2 + 1], palette=color)
        axes[index * 2 + 1].set_title(f"Countplot of {column} specified by {target_select}")
